...
class HttpClient:
    ...
    def _make_request(self, method: str, url: str, **kwargs) -> Dict[str, Any]:
        ...
        if isinstance(json_response, dict):
            if (
                json_response.get("status") == "error"
                and "error_message" in json_response
            ):
                error_details = json_response["error_message"]
                raise LLMStreamingException(f"Server returned error: {error_details}")
            elif "error" in json_response and isinstance(json_response["error"], dict):
                error_details = json_response["error"].get("message", "Unknown error")
                raise LLMStreamingException(f"API returned error: {error_details}")
    ...
    def _make_raw_request(self, method: str, url: str, **kwargs) -> requests.Response:
        ...
        try:
            ...
            # Intentar extraer error_message del contenido de la respuesta
